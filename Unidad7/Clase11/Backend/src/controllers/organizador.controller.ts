...
export const reporteOrganizador = (req: Request, res: Response) => {
    let cantidad_masculino:number = 0;
    let cantidad_femenino:number = 0;
    for(let i:number = 0; i < lista_organizadores.length; i++){
        if(lista_organizadores[i].getGenero().toUpperCase() === "M"){
            cantidad_masculino++;
        }else{
            cantidad_femenino++;
        }
    ...
    res.json({
        mensaje: 'Reporte de organizadores',
        masculino: cantidad_masculino,
        femenino: cantidad_femenino,
        type: 'success',
        status: 200
    })
}
